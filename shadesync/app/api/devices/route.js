...
export async function GET() {
    ...
    return Response.json({ device: data }, { status: 200 });
}
...
export async function POST(req) {
    ...
    if (!serialNumber) return Response.json({ error: "Missing serialNumber "}, { status: 400 });
    ...
    if (!device) return Response.json({ error: "Invalid serial" }, { status: 400 });
    ...
    const { error: claimErr } = await supabaseService
        .from("devices")
        .update({ owner_id: session.user.id, registered_at: new Date().toISOString() })
        .eq("id", device.id)
    if (claimErr) return Response.json({ error: claimErr.message }, { status: 400 });
